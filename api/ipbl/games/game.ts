...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const official = await fetchOfficialJson("/games/game", query);
  if (official) {
  ...
  if (!resolved) return res.status(404).json({ error: "Game not found in official or verified fallback sources", id, tag });
  ...
  return res.status(200).json(buildGameDetailPayload(resolved.game, resolved.source));
}
