...
const h = parsed.games.find((game) => game.team1.shortName === "Belgorod")!;
assert.equal(h.tag, "ipbl-74-m-pro-h");
assert.equal(h.divisionLabel, "Pro Men H");
assert.equal(h.team1.teamId, 8630740, "live-only H must preserve source identity without fabricating official IDs");
assert.equal(h.timeToGo, "02:33");
